		...
		if(!reset_n) begin 
			...
			case(state_encr) 
				...
				ENCRYPT: begin
					...
					lfsr_m <= {lfsr_m[62:0], lfsr_m[63] ^ lfsr_m[43] ^ lfsr_m[33] ^ lfsr_m[22] ^ lfsr_m[11] ^ lfsr_m[0]};
					lfsr_s <= {lfsr_s[62:0], lfsr_s[63] ^ lfsr_s[43] ^ lfsr_s[33] ^ lfsr_s[22] ^ lfsr_s[11] ^ lfsr_s[0]};

					encypt_count <= encypt_count + 1;

    					$display("[CRYPTO] ENCRYPT: encypt_count=%d", encypt_count);
					...
					if( encypt_count == 63) begin
						...
						if(is_write) encrypt_text_reg <= plain_text ^ lfsr_m;		//write
						else encrypt_text_reg <= slave_rx ^ lfsr_s;			//read
